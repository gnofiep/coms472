		...
		while (!OPEN.isEmpty())
		{
			if (searchStrategy.equalsIgnoreCase("beam")) {
				if (OPEN.size() > beamWidth) {
					LinkedList<SearchNode> cropList = new LinkedList<SearchNode>();
					for(int i = 0;i<beamWidth;i++)
						cropList.add(OPEN.get(i));
					OPEN.clear();
					OPEN.addAll(cropList);
				}
			}
...
//				currentNode.reportSolutionPath();
		...
		System.out.println("Length of traversed path is " + pathLength + ".\n Path list is: " + path);
	}
	...
	static double heuristicValue(String contents, String hypertext) {
		...
		Stoken = new StringTokenizer(hypertext);
		while(Stoken.hasMoreTokens()){
			word = Stoken.nextToken();
		...
		while(Stoken.hasMoreTokens()){
			...
			if(count!=0 && !conWord) {
				if(count == 1)
					conQueryValue = conQueryValue + count;
				else
					conQueryValue = conQueryValue + 2*count;
				count = 0;
			}
		}

		double total = (1*queryWord) + (2*queryHypTxt) + (4*conQueryValue);
		if(total == 0.0)
			return 1;

		double heuristic = (double)1/total;
...
class SearchNode
{
	...
	public List<String> reportSolutionPath() {
		SearchNode sNode = this;
		List<String> pathList = new ArrayList<String>();
		while(sNode != null) {
			pathList.add(sNode.getNodeName());
			sNode = sNode.parent;
		}
		return pathList;
	}
